...
export function generateEmailHTML(changes: PriceChange[], timestamp: string): string {
  ...
  const summary = hasChanges
    ...
    : `<p style="font-size: 16px; color: #1f2937;">
         ✅ No pricing changes detected today. Everything looks stable!
       </p>`;
  ...
  const actionsHTML = hasChanges
    ...
    : `
      <div style="margin-top: 25px; padding: 20px; background: #f0fdf4; border-radius: 8px; border: 1px solid #bbf7d0;">
        <strong style="font-size: 16px; color: #166534;">✅ All Good!</strong>
  ...
  return `
    ...
    <html>
      ...
      <body>
        <div class="container">
          <div class="header">
            <h1>📊 LLM PriceCheck Daily Report</h1>
...
export async function sendDailyReport(
  ...
  if (result.success) {
    ...
    console.log('⚠️  Email not sent (API key not configured)');
  }
}
